# Agent rules — strata_forge.training

`strata_forge.training` ships fine-tuning primitives: SFT, preference
...
- **Does NOT import** :mod:`strata_forge.tracing`, :mod:`strata_forge.agents`,
  ...
  :mod:`strata_forge.compute` live in ``examples/`` or in
...
- **Heavy imports stay inside ``train``.** A top-level
  ``import transformers`` would break the "import ``strata_forge.training``
