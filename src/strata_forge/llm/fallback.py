...
- **Inner (provider-level)**: within one :class:`ModelFallback`, the
  ...
  :func:`strata_forge.core.retry.retry` (exponential backoff + jitter) so a
...
from strata_forge.core.retry import retry as retry_decorator
from strata_forge.llm.routing import resolve
...
async def run_with_fallback[T](
    ...
    *,
    ...
    Each attempt is wrapped in :func:`strata_forge.core.retry.retry` so a
