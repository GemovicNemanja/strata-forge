...
When ``FORGE_DIAGNOSTIC_ENABLED=1`` (via :class:`strata_forge.config.DiagnosticConfig`),
...
class DiagnosticRecord(BaseModel):
    ...
    correlation_id: str | None = Field(
        default=None,
        description="The active `strata_forge.core.ids.correlation_id`, if set.",
    )
    request_hash: str = Field(
        description=(
            "Provider-agnostic SHA-256 of the request — typically produced "
            "by `strata_forge.llm.cache.cache_key`."
        )
    )
