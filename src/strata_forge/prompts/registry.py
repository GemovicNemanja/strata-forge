...
:mod:`strata_forge.prompts.stores.memory`, a Langfuse-backed one ships in
:mod:`strata_forge.prompts.stores.langfuse`. The :class:`PromptRegistry` layer
on top adds two things stores shouldn't have to repeat:

- **Declaration validation.** Every :meth:`PromptRegistry.put` runs
  :func:`strata_forge.prompts.variables.validate_template_variables` before
...
- **Lint signal.** When the template declares a stable section but
  ...
  :data:`strata_forge.prompts.cache_aware.DEFAULT_MIN_CACHEABLE_TOKENS`, the
...
from strata_forge.prompts.cache_aware import StableDynamicSplit, is_stable_too_short
...
class PromptRegistry:
    ...
    implementation of your choice::
        ...
        from strata_forge.prompts import PromptRegistry
    ...
    def __init__(self, store: PromptStore) -> None:
        self._store = store
        self._logger = get_logger("strata_forge.prompts.registry")
