...
The async :class:`strata_forge.llm.LLMClient` is the canonical interface — every
...
Construction style: pass an existing :class:`~strata_forge.llm.LLMClient`
...
.. warning::
    ...
    :meth:`strata_forge.llm.LLMClient.stream` from an async context if you
...
if TYPE_CHECKING:
    ...
    from strata_forge.llm.responses import LLMResponse, ResponseChunk
...
def complete(
    ...
    **kwargs: Any,
    ...
    """Synchronous :meth:`~strata_forge.llm.LLMClient.complete`.
    ...
    :class:`~strata_forge.llm.LLMResponse`.
    """
...
def complete_structured[M: BaseModel](
    ...
    **kwargs: Any,
    ...
    """Synchronous :meth:`~strata_forge.llm.LLMClient.complete_structured`.

    Returns the same :class:`~strata_forge.llm.StructuredResponse` — call
...
def stream(
    ...
    **kwargs: Any,
    ...
    """Synchronous wrapper that drains :meth:`~strata_forge.llm.LLMClient.stream`.
...
def run_tool_loop(
    ...
    **kwargs: Any,
    ...
    """Synchronous :meth:`~strata_forge.llm.LLMClient.run_tool_loop`."""
    target = _resolve_client(client=client, model=model, provider=provider, chain=chain)
    return asyncio.run(
