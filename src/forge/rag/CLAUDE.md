# Agent rules — forge.rag

`forge.rag` is the retrieval-augmented generation layer. It owns
...
- :class:`VectorStore` Protocol + :class:`InMemoryVectorStore` +
  ...
  storage primitives. Relocated from :mod:`forge.agents.memory`
...
- **Imports from inside ``forge``:** :mod:`forge.core` (errors,
  ids), :mod:`forge.config` (settings for production backends
  when they need them). May import :mod:`forge.llm` for token
  counting in chunkers, but **does NOT depend on**
  :mod:`forge.llm.LLMClient` — embeddings go through ``litellm``
  directly (ADR 0012).
- **Does NOT import** :mod:`forge.tracing` (ADR 0008),
  :mod:`forge.agents`, :mod:`forge.evals`,
  :mod:`forge.datasets`. The dependency arrow points downward:
...
- **No new LLM client.** Embeddings call ``litellm.aembedding``
  ...
  path on :class:`forge.llm.LLMClient` is untouched.
...
- **Don't duplicate :class:`VectorStore` in another module.** It
  used to live in :mod:`forge.agents.memory`; ADR 0012 moved it
