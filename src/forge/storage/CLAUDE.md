# Agent rules — forge.storage

`forge.storage` is the file-storage and Hugging Face Hub layer.
...
- **Imports from inside ``forge``:** nothing — this module is a
  pure I/O leaf. Higher-level modules (``forge.datasets``,
  ``forge.training``, ``forge.compute``) consume
  :mod:`forge.storage`, never the other way around.
- **External deps:** ``fsspec`` plus its protocol backends
  ...
  on first use so ``import forge.storage`` works without the
  extra.
