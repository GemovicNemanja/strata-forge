# Agent rules — forge.evals

`forge.evals` is the experiment runner — it composes models, prompts,
...
- **Owns:** `experiment.py`, `graders/`. The runner (2.4.2),
  ...
  later sub-phases.
- **Imports from inside `forge`:** `forge.core`, `forge.config`,
  `forge.llm`, `forge.prompts`, `forge.datasets`. Does NOT import
  `forge.tracing` (tracing is layered above, per ADR 0008).
...
- **Don't import provider SDKs.** All LLM activity goes through
  ``forge.llm.LLMClient`` — graders that need a model take an
