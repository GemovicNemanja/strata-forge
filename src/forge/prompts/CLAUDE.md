# Agent rules — forge.prompts

`forge.prompts` is the typed prompt authoring layer. Templates are
...
`list[forge.llm.AnyMessage]`. The module owns:
...
- Rendering to `list[forge.llm.AnyMessage]` with the cache hints the
...
- **Owns:** `template.py`, `cache_aware.py`, `variables.py`,
  ...
  `rendering.py`.
- **Imports from inside `forge`:** `forge.core` (errors, logging,
  reproducibility helpers) and `forge.llm` (message types and the
...
- `render(template, variables)` — produces `list[AnyMessage]` plus the
  cache hints `forge.llm` consumes.
...
- **Do not log full rendered prompts at `INFO`.** They contain user
  data. Use `DEBUG`; the NDJSON diagnostic dump in `forge.llm` is the
  appropriate channel.
