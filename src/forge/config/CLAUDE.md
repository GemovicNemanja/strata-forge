# Agent rules — forge.config

`forge.config` is the single source of truth for runtime configuration. It depends only on `forge.core`.
...
- **Imports from inside `forge`:** `forge.core` only.
...
- Adding a new sub-model (e.g. `forge.config.settings.RagConfig`).
