...
:class:`forge.config.LangfuseConfig`. The function returns ``None`` in
three cases — Langfuse credentials aren't configured, the
``[langfuse]`` extra isn't installed, or the constructor itself
failed — so every public function in :mod:`forge.tracing` can
...
def get_client() -> Any:
    ...
    :mod:`forge.tracing` is importable without the ``[langfuse]``
