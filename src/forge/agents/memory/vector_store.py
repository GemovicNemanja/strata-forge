...
agent runs. :mod:`forge.rag` (Phase 4) will add a Qdrant-backed
implementation that also satisfies the Protocol — agents written
against the Protocol need no changes to swap stores.
"""
...
class VectorStore(Protocol):
    ...
    Async by design — the Qdrant and other production backends in
    :mod:`forge.rag` are async-native.
    """
...
def _cosine_similarity(a: Sequence[float], b: Sequence[float]) -> float:
    ...
    default for undefined direction.
    """
    if len(a) != len(b):
