...
:mod:`forge.compute` runs. :class:`SFTRunner` wires the config
together with a :class:`forge.training.peft.LoRAConfig` or
:class:`forge.training.peft.QLoRAConfig` (optional) and invokes
TRL.

All heavy imports — ``torch``, ``transformers``, ``trl``,
``datasets`` — are deferred until :meth:`SFTRunner.train` is
called, so importing :mod:`forge.training.sft` succeeds without
...
class SFTConfig(BaseModel):
    ...
    Attributes:
        ...
        packing: Use TRL's packing (different from Forge's
            :mod:`forge.training.packing` — that one is for offline
            preprocessing).
