...
``forge.training`` runners are synchronous and only return once training
finishes; :class:`forge.compute.Backend` exposes ``logs`` (a *pull*) but no push
...
def trainer_callback(writer: JsonlProgressWriter) -> Any:
    ...
    :mod:`forge.training.progress` never pulls the ``[finetuning]`` extra. The
