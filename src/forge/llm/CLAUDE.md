# Agent rules — forge.llm

`forge.llm` is the central LLM abstraction. Every higher-level module that talks to a model goes through here.
...
The `forge.sync` namespace re-exports `complete`, `stream`, `complete_structured`, `run_tool_loop` as sync wrappers.
...
- Adding a new provider module (e.g. `forge.llm.providers.mistral`).
