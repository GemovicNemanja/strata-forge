...
class ForgeError(Exception):
    ...
    Every exception raised from ``forge.*`` code paths inherits from this class so
...
class ProviderError(ForgeError):
    ...
    ...). The seam in ``forge.llm.errors.map_litellm_exception`` is responsible
...
class BudgetExceededError(ForgeError):
    ...
    Raised by ``forge.core.budget.BudgetContext`` before the budget is actually
