# Agent rules — forge.cli

`forge.cli` is the Typer-based command-line surface for the
...
- **Imports from inside ``forge``:** every other module —
  ``forge.cli`` is the top of the dependency arrow.
...
- **Heavy modules import lazily inside command bodies.** Don't
  import :mod:`forge.training` or :mod:`forge.compute.backends`
...
- **Don't add command-line flags that duplicate module-level
  Pydantic config.** Configs flow through ``forge.config`` and
...
- **``forge.llm.registry.registry`` is a Registry instance, not a
  module.** When tests monkeypatch it, they target
  ``sys.modules["forge.llm.registry"]``, not
  ``import forge.llm.registry as m`` (the latter binds to the
