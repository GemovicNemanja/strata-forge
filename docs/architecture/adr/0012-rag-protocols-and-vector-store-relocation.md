...
**Status:** Accepted
**Date:** Initial scaffolding for `forge.rag`
**Supersedes:** —
**Superseded by:** —
...
:mod:`forge.agents.memory` so that :class:`EpisodicMemory` had
...
already anticipated this: it described `forge.agents` as "imports
from `forge.rag` when its vector-store Protocol lands." Phase 4.1
...
`forge.rag` (with concrete impls in `forge.rag` and elsewhere), or
does it live in some other module?

## Decision

### 1. The vector-store primitives relocate to `forge.rag`

`VectorStore`, `VectorItem`, `VectorSearchResult`, the
in-process `InMemoryVectorStore`, and `cosine_similarity` all
move from :mod:`forge.agents.memory.vector_store` to
:mod:`forge.rag.vector_store`.

`forge.agents.memory` re-exports the relocated names from
:mod:`forge.rag` so the agent-facing public API (`from
forge.agents import VectorStore, InMemoryVectorStore, …`) keeps
working. `forge.agents.memory.episodic` imports them from
:mod:`forge.rag`. The dependency arrow points one way:
`agents → rag`.
...
# VectorStore Protocol relocated from forge.agents.memory; same shape.
```
...
`forge.llm.LLMClient` doesn't grow an `aembedding`-style method.
...
Instead, `forge.rag.LiteLLMEmbedder` calls `litellm.aembedding`
...
Same shape rules as :mod:`forge.datasets` and :mod:`forge.evals`:
...
- **One source of truth.** Adding a new VectorStore backend
  (Weaviate, pgvector) means one place: `forge.rag.stores.*`. The
...
- **One-time migration.** Existing code that imports
  `VectorStore` etc. from `forge.agents.memory` still works
  (re-exports), but the canonical path is `forge.rag`. The
  re-exports are documented in :mod:`forge.agents.memory` and we
...
- **`LiteLLMEmbedder` duplicates litellm-wrapping work** that
  ...
  cache backend from `forge.llm.cache`) can be added in later
  RAG sub-phases if the demand surfaces.

## Alternatives considered

1. **Keep `VectorStore` in `forge.agents.memory`; have `forge.rag`
   import from `forge.agents`.** Inverts the dependency arrow
   from the CLAUDE.md spec. `forge.rag` would become a leaf module
   that depends on `forge.agents` — wrong direction for the
   layered architecture. Rejected.

2. **Put `VectorStore` in `forge.core`.** Maximally neutral but
