...
**`main` only ever receives `dev` — never a topic branch, never a direct push.** Enforced, not just convention: `main` is protected (PR required, direct pushes blocked, no admin bypass) and a required CI check (`.github/workflows/promotion-guard.yml`) **fails any PR into `main` whose source branch isn't `dev`**. Agents take a change as far as `dev` and never push to `main`.
